...
func enter_monkeys():
	...
	for i in range(monkeys_to_spawn):
		...
		(
			tween
			. tween_property(
				monkey, "global_position", Vector2(target_x, monkey.global_position.y), 1.5
			)
...
func monkeys_grab_aj():
	...
	rotate_tween.tween_property(aj, "rotation", 1.5, 1).set_trans(Tween.TRANS_SINE).set_ease(
		Tween.EASE_IN_OUT
	)
	...
	(
		move_tween
		. tween_property(aj, "global_position", target_position, 0.8)
		. set_trans(Tween.TRANS_SINE)
		. set_ease(Tween.EASE_IN_OUT)
	)
...
func monkeys_exit():
	...
	for i in range(monkeys.size()):
		...
		(
			...
			. parallel()
			. tween_property(
				monkey, "global_position", Vector2(exit_x, monkey.global_position.y), 2.0
			)
			. set_trans(Tween.TRANS_SINE)
			. set_ease(Tween.EASE_IN_OUT)
		)
	...
	if monkeys.size() > 2:
		...
		(
			tween
			. parallel()
			. tween_property(aj, "global_position", aj_target, 2.0)
			. set_trans(Tween.TRANS_SINE)
			. set_ease(Tween.EASE_IN_OUT)
		)
...
func cb_exits(player):
	...
	(
		tween
		. tween_property(player, "global_position", Vector2(exit_x, player.global_position.y), 2.0)
		. set_trans(Tween.TRANS_SINE)
		. set_ease(Tween.EASE_IN_OUT)
	)
	await tween.finished
