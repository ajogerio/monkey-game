...
func _process(delta: float) -> void:
	...
	if (
		player_node.global_position.x > right_barrier_of_deadzone
		and global_position.x < max_camera_x
	):
		global_position.x += player_node.velocity.x * delta
	elif (
		player_node.global_position.x < left_barrier_of_deadzone
		and global_position.x > min_camera_x
	):
		global_position.x += player_node.velocity.x * delta
